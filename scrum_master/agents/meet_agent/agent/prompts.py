...
4. Извлеки структурированные данные с ТОЧНЫМИ дедлайнами и приоритетами. Собеби извлеченные данные в внутреннюю структуру meeting_data (в формате JSON, описанном ниже). НЕ выводи структуру meeting_data пользователю!
5. **Сразу после анализа и сборки meeting_data** вызови инструменты:
    - send_meeting_report(meeting_data=твой_JSON_структура)
    - export_to_notion(meeting_data=твой_JSON_структура)
    - **ДЛЯ КАЖДОЙ ЗАДАЧИ** из списка tasks вызови create_jira_issue() или update_jira_issue().
    Это отправит PDF в Telegram, создаст страницы в Notion и обновит Jira. Не выводи ничего другого после этого.
...
6. **Финальные шаги**:
   ...
   - Для каждой новой задачи: create_jira_issue(...)
   - Для обновлений существующих задач (если найдены ключи задач): update_jira_issue(...)
   
   НЕ выводи JSON пользователю!

7. **Интеграция с Jira**:
   - Используй `create_jira_issue` для создания новых задач.
   - Поля для Jira:
     * `summary`: используй `title` из задачи.
     * `description`: объедини `description`, `context` и `priority_reason` для полноты картины.
     * `assignee`: используй имя исполнителя (если есть).
     * `priority`: используй `priority` (High, Medium, Low).
     * `duedate`: используй `deadline` (YYYY-MM-DD).
   - Если в разговоре упоминается обновление существующей задачи (например, "в задаче PROJ-123 поменяй приоритет"), используй `update_jira_issue`.
